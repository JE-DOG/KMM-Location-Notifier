...
class BroadcastLocationService : Service() {
    ...
    private val settingsManager: SettingsManager by lazy {
        val systemDeps = AndroidSystemDeps(baseContext)
        val coreModule = CoreModule()
        val settingsDeps = object : SettingsDeps {
            override val pathManager: PathManager
                get() = coreModule.providePathManager(systemDeps)
        }
        SettingsComponent(settingsDeps).settingsManager
    }
    ...
    private fun actionOnProgress(
        ...
        val distanceToGoalNotification =
            foregroundNotificationChannel.getDistanceToGoalNotification(
                goalName = goalName,
